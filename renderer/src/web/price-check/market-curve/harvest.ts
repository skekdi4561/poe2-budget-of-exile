// 소극 수집 — 사용자가 스스로 한 가격 검색의 응답에서 공격 무기 7종·방패 매물만 추려
// 수합 서버에 보낸다. 추가 거래소 API 호출은 0회다(이미 도착한 응답 재활용).
// 사용자·판매자 계정명은 보내지 않는다 — 공개 매물 수치만. 단 매물 id 는 거래소에서
// 판매자로 이어지는 공개 식별자라 "익명"은 아니다(README 고지). 설정에서 끌 수 있다.
